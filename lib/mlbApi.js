 ...
 *
 * Base weights:
 *   starterSIERA   20% — opposing starter SIERA (FanGraphs)
 *   starterXFIP    20% — opposing starter xFIP (FanGraphs)
 *   starterERAplus 10% — opposing starter ERA+ (MLB API, lazy)
 *   h2h            10% — career BA vs opposing starter (MLB API, lazy)
 *   batterSplitOPS 10% — batter OPS vs same handedness (MLB API, lazy)
 *   batterForm     10% — 7-day H+R+RBI
 *   batterWRC       5% — batter wRC+ (FanGraphs)
 *   batterBABIP     5% — batter BABIP (FanGraphs)
 *   bullpenERA      5% — opposing bullpen ERA
 *   pitchMix        5% — placeholder, always 50 (Coming Soon)
 *
 * Redistribution:
 *   SIERA missing  → xFIP +10%, ERA+ +10%
 *   xFIP missing   → SIERA +10%, ERA+ +10%
 *   H2H < 5 AB     → SIERA +5%, xFIP +5%
 *   wRC+ missing   → 7-day form +5%
 *   BABIP missing  → 7-day form +5%
 */
export function computeBatterMatchupScore({
  ...
  if (!sieraAvail) { wSIERA = 0; wXFIP  += 10; wERAplus += 10 }
  if (!xfipAvail)  { wXFIP  = 0; wSIERA += 10; wERAplus += 10 }
  if (!h2hSufficient)        { wH2h   = 0; wSIERA  +=  5; wXFIP += 5 }
  if (batterWRC   == null)   { wWRC   = 0; wForm   +=  5 }
  ...
  let eraPlusScore = null
  if (starterERAplus != null) {
    if (starterERAplus > 130) eraPlusScore = 0
    else if (starterERAplus >= 100) eraPlusScore = 30
    else if (starterERAplus >= 80)  eraPlusScore = 60
