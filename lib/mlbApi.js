...
/**
 * New 6-component batter matchup grade (totals 100%).
 *
 * Weights (base):
 *   batterForm  25%  — 7-day H+R+RBI
 *   h2h         20%  — career vs starter (redistributed when < 5 AB)
 *   batterWRC   15%  — batter wRC+ from FanGraphs
 *   starterXFIP 15%  — opposing starter xFIP from FanGraphs
 *   bullpenERA  15%  — opposing bullpen ERA
 *   starterKPct 10%  — opposing starter K% from FanGraphs
 *
 * Insufficient H2H (<5 AB): 20% H2H redistributed → +10% xFIP, +5% bullpen, +5% form.
 * Missing FanGraphs component: its weight redistributed proportionally across available.
 */
export function computeBatterMatchupScore({
  ...
  // 7-day form: max useful total ~18
  const formScore = clamp((batterForm / 18) * 100, 0, 100)

  // H2H career vs starter
  let h2hScore = null
  if (h2hSufficient && vsPlayer) {
    ...
    h2hScore = Math.min(avgScore + hrBonus + rbiBonus, 100)
  }

  // Batter wRC+ (100 = avg; 140+ = excellent → 100, 60 = poor → 0)
  const wrcScore = batterWRC != null
    ? clamp(((batterWRC - 60) / 80) * 100, 0, 100)
    : null

  // Opposing starter xFIP (lower = tougher for batter; 2.5→0, 6.0→100)
  const xfipScore = starterXFIP != null
    ? clamp(((starterXFIP - 2.5) / 3.5) * 100, 0, 100)
    : null

  // Bullpen ERA (higher = better for batter)
  const bullpenScore = clamp(((bullpenERA - 1.5) / 5.5) * 100, 0, 100)

  // Opposing K% (higher = tougher for batter; 10%→100, 40%→0)
  const kPctScore = starterKPct != null
    ? clamp((1 - (starterKPct - 10) / 30) * 100, 0, 100)
    : null
  ...
  const comps = [
    ...
    { key: 'starterKPct', score: kPctScore,    weight: wKPct    },
  ]
