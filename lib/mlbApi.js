...
/**
 * New 10-component batter matchup grade (weights total 100%).
 *
 * Base weights:
 *   starterSIERA    5% — opposing starter SIERA (FanGraphs)
 *   starterXFIP     5% — opposing starter xFIP (FanGraphs)
 *   starterERAplus 10% — opposing starter ERA+ (MLB API, lazy)
 *   h2h            15% — career BA vs opposing starter (MLB API, lazy)
 *   batterSplitOPS 15% — batter OPS vs same handedness (MLB API, lazy)
 *   batterForm     20% — 7-day H+R+RBI
 *   batterWRC      10% — batter wRC+ (FanGraphs)
 *   batterBABIP     5% — batter BABIP (FanGraphs)
 *   bullpenERA     10% — opposing bullpen ERA
 *   pitchMix        5% — placeholder, always 50 (Coming Soon)
 *
 * Redistribution:
 *   SIERA missing             → xFIP +5% (now 10%)
 *   xFIP missing              → SIERA +5% (now 10%)
 *   Both SIERA & xFIP missing → ERA+ +10% (now 20%)
 *   H2H < 5 AB                → 7-day form +8% (now 28%), splits +4% (now 19%), ERA+ +3% (now 13%)
 *   wRC+ missing              → 7-day form +10% (now 30%)
 *   BABIP missing             → 7-day form +5% (now 25%)
 */
export function computeBatterMatchupScore({
  starterSIERA   = null,
  starterXFIP    = null,
  starterERAplus = null,
  vsPlayer       = null,
  ...
  // Redistribution
  const sieraAvail = starterSIERA != null
  const xfipAvail  = starterXFIP  != null
  if (!sieraAvail && !xfipAvail) { wSIERA = 0; wXFIP = 0; wERAplus += 10 }
  else if (!sieraAvail)          { wSIERA = 0; wXFIP  +=  5 }
  else if (!xfipAvail)           { wXFIP  = 0; wSIERA +=  5 }
  if (!h2hSufficient)  { wH2h  = 0; wForm += 8; wSplitOPS += 4; wERAplus += 3 }
  if (batterWRC  == null) { wWRC  = 0; wForm += 10 }
  if (batterBABIP == null) { wBABIP = 0; wForm +=  5 }

  // Runtime assertion: weights must always sum to 100
  const _weightSum = wSIERA + wXFIP + wERAplus + wH2h + wSplitOPS + wForm + wWRC + wBABIP + wBullpen + wPitchMix
  if (_weightSum !== 100) console.warn(`computeBatterMatchupScore: weights sum to ${_weightSum}, expected 100`)

  // ── Component scores ──────────────────────────────────────────────────────

  // 1. Starter SIERA (lower = worse for batter)
  function sieraXFIPScore(val) {
    if (val == null) return null
    if (val < 3.00) return 0
    if (val < 3.50) return 20
    if (val < 4.00) return 40
    if (val < 4.50) return 60
    if (val < 5.00) return 80
    return 100
  }
  const sieraScore = sieraXFIPScore(starterSIERA)
  const xfipScore  = sieraXFIPScore(starterXFIP)

  // 3. Starter ERA+ (higher ERA+ = worse for batter)
  let eraPlusScore = 45  // neutral fallback when unavailable
  if (starterERAplus != null) {
    if (starterERAplus > 120)       eraPlusScore = 0
    else if (starterERAplus >= 100) eraPlusScore = 20
    else if (starterERAplus >= 80)  eraPlusScore = 45
    else if (starterERAplus >= 60)  eraPlusScore = 70
    else                             eraPlusScore = 100
  }

  // 4. Career BA vs opposing starter
  let h2hScore = null
  if (h2hSufficient && vsPlayer) {
    const avg = vsPlayer.ab > 0 ? vsPlayer.h / vsPlayer.ab : 0
    if (avg > 0.300)      h2hScore = 100
    else if (avg >= 0.250) h2hScore = 75
    else if (avg >= 0.200) h2hScore = 50
    else if (avg >= 0.150) h2hScore = 25
    else                   h2hScore = 0
  }

  // 5. Batter OPS vs same-handedness
  let splitScore = null
  if (batterSplitOPS != null) {
    if (batterSplitOPS > 0.900)      splitScore = 100
    else if (batterSplitOPS >= 0.800) splitScore = 75
    else if (batterSplitOPS >= 0.700) splitScore = 50
    else if (batterSplitOPS >= 0.600) splitScore = 25
    else                               splitScore = 0
  }

  // 6. 7-day H+R+RBI form (relative to today's player median — scored at call site)
  //    Here we receive a pre-normalized 0-100 score or raw total; treat raw total
  //    with max useful total ~18 as fallback when no median available.
  const formScore = clamp(
    typeof batterForm === 'number' ? (batterForm / 18) * 100 : 0,
    0, 100
  )
  ...
  let wrcScore = null
  if (batterWRC != null) {
    if (batterWRC > 150)       wrcScore = 100
    else if (batterWRC >= 125) wrcScore = 80
    else if (batterWRC >= 100) wrcScore = 60
    else if (batterWRC >= 75)  wrcScore = 40
    else                        wrcScore = 20
  }

  // 8. Batter BABIP (higher = better for batter)
  let babipScore = null
  if (batterBABIP != null) {
    if (batterBABIP > 0.350)       babipScore = 100
    else if (batterBABIP >= 0.320) babipScore = 75
    else if (batterBABIP >= 0.290) babipScore = 50
    else if (batterBABIP >= 0.260) babipScore = 25
    else                            babipScore = 0
  }

  // 9. Opposing bullpen ERA (higher = better for batter)
  let bullpenScore = 50
  if (bullpenERA > 5.00)       bullpenScore = 100
  else if (bullpenERA >= 4.00) bullpenScore = 75
  else if (bullpenERA >= 3.50) bullpenScore = 50
  else if (bullpenERA >= 3.00) bullpenScore = 25
  else                          bullpenScore = 0

  // 10. Pitch mix — placeholder
  // TODO: Replace with Baseball Savant pitch mix data in future pass
  const pitchMixScore = 50

  const comps = [
    { key: 'starterSIERA',   score: sieraScore,    weight: wSIERA    },
    { key: 'starterXFIP',    score: xfipScore,     weight: wXFIP     },
    { key: 'starterERAplus', score: eraPlusScore,  weight: wERAplus  },
    { key: 'h2h',            score: h2hScore,      weight: wH2h      },
    { key: 'batterSplitOPS', score: splitScore,    weight: wSplitOPS },
    { key: 'batterForm',     score: formScore,     weight: wForm     },
    { key: 'batterWRC',      score: wrcScore,      weight: wWRC      },
    { key: 'batterBABIP',    score: babipScore,    weight: wBABIP    },
    { key: 'bullpenERA',     score: bullpenScore,  weight: wBullpen  },
    { key: 'pitchMix',       score: pitchMixScore, weight: wPitchMix },
  ]

  // Proportional weighting among components with available scores
  const totalWeight = comps.reduce((s, c) => s + (c.score != null ? c.weight : 0), 0)
  const composite   = totalWeight > 0
    ? comps.reduce((s, c) => c.score != null ? s + c.score * (c.weight / totalWeight) : s, 0)
    : 50

  const components = {}
  for (const c of comps) components[c.key] = { score: c.score, weight: c.weight }
  components.h2h = { ...components.h2h, sufficient: h2hSufficient }

  return { score: Math.round(composite), components }
}

/**
 * New 6-component pitcher matchup grade (totals 100%).
 * High score = favorable start (pitcher advantage). A = great, F = avoid.
 *
 * Weights:
 *   pitcherXFIP   25%  — from FanGraphs (lower = better)
 *   pitcherSIERA  20%  — from FanGraphs (lower = better)
 *   opposingWRC   20%  — opposing lineup avg wRC+ (lower = better for pitcher)
 *   kMinusBB      15%  — K% − BB% from FanGraphs (higher = better)
 *   last3          15%  — last 3 starts avg ERA + WHIP (lower = better)
 *   opposingHard    5%  — opposing lineup Hard% (lower = better)
 */
export function computePitcherMatchupScore({
  pitcherXFIP    = null,
  pitcherSIERA   = null,
  opposingWRC    = null,
  pitcherKPct    = null,
  pitcherBBPct   = null,
  last3StartsAvg = null,
  opposingHardPct = null,
}) {
  const w = { pitcherXFIP: 25, pitcherSIERA: 20, opposingWRC: 20, kMinusBB: 15, last3: 15, opposingHard: 5 }

  // xFIP (2.0→100, 4.0→50, 6.0→0)
  const xfipScore = pitcherXFIP != null
    ? clamp((1 - (pitcherXFIP - 2.0) / 4.0) * 100, 0, 100)
    : null

  // SIERA (same scale as xFIP)
  const sieraScore = pitcherSIERA != null
    ? clamp((1 - (pitcherSIERA - 2.0) / 4.0) * 100, 0, 100)
    : null

  // Opposing wRC+ (70→100 great, 100→62.5 avg, 150→0 tough)
  const wrcScore = opposingWRC != null
    ? clamp((1 - (opposingWRC - 70) / 80) * 100, 0, 100)
    : null

  // K% − BB% net (0→0, 12.5→50, 25+→100)
  let kMinusBBScore = null
  if (pitcherKPct != null && pitcherBBPct != null) {
    kMinusBBScore = clamp(((pitcherKPct - pitcherBBPct) / 25) * 100, 0, 100)
  }

  // Last 3 starts avg ERA + WHIP
  let last3Score = null
  if (last3StartsAvg) {
    const eraNorm  = clamp((1 - (last3StartsAvg.era  - 1.5) / 7.0) * 100, 0, 100)
    const whipNorm = clamp((1 - (last3StartsAvg.whip - 0.7) / 1.8) * 100, 0, 100)
    last3Score = (eraNorm + whipNorm) / 2
  }

  // Opposing Hard% (20→100, 32.5→50, 45→0)
  const hardScore = opposingHardPct != null
    ? clamp((1 - (opposingHardPct - 20) / 25) * 100, 0, 100)
    : null

  const comps = [
    { key: 'pitcherXFIP',  score: xfipScore,     weight: w.pitcherXFIP  },
    { key: 'pitcherSIERA', score: sieraScore,     weight: w.pitcherSIERA },
    { key: 'opposingWRC',  score: wrcScore,       weight: w.opposingWRC  },
    { key: 'kMinusBB',     score: kMinusBBScore,  weight: w.kMinusBB     },
    { key: 'last3',        score: last3Score,     weight: w.last3        },
    { key: 'opposingHard', score: hardScore,      weight: w.opposingHard },
  ]
 ...
 * A=top 8%, B=8-25%, C=25-65%, D=65-82%, F=bottom 18%
 */
